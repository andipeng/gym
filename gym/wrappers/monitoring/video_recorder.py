...
class VideoRecorder(object):
    ...
    def _label_with_reward(self, frame, rew):
        ...
        if np.mean(im) < 128:
            ...
            text_color = (0, 0, 0)
        drawer.text((im.size[0]/20, im.size[1]/18), f'Reward: {rew}', fill=text_color)

        return im

    def capture_frame(self, rew=None):
        ...
        if rew is not None:
            frame = np.array(self._label_with_reward(frame, rew))
